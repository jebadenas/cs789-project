#!/bin/bash
# Step 4 (aggregate per cascade state) on the 72B: group the notes by state and
# find recurring patterns. Only 8 calls, so quick.
#
#   sbatch slurm/journal_aggregate.sh
#
#SBATCH --job-name=journal-aggregate
...
python3 -m src.qualitative.llm.run aggregate
